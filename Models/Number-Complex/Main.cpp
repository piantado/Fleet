...
double ANSzero(const double n1, const double n2) {
	...
	double mu = abs(n1-n2);
	double sd = W*sqrt(n1*n1+n2*n2+0.01); // +0.01 so we can compare zeros?
	
	// compute prob. in [-0.5, 0.5]
	return normal_cdf(0.5, mu, sd) - normal_cdf(-0.5, mu, sd);
}

double ANSdiff(const double n1, const double n2) {  // SD of the weber value
	return 1.0-normal_cdf(0.0, abs(n1-n2), W*sqrt(n1*n1+n2*n2));
}
...
MyHypothesis::datum_t sample_datum(int w) {
	// this uses a little rejection sampling in order to find a datum
	// with number w (NOTE: it might be an incorrect one...)
	while(true) {
		auto d = sample_datum();
		if(d.output == w) return d; 
	}
}
...
//		MCMCChain samp(h0, &alldata[di]);
	...
	{
		// After the third session (i.e., the mid‐test), children were given a total of 13 trials with set
		// sizes one and two presented once, sets of three, four, and five presented three times, and six presented twice.

		data_t newdata = biggestData; // use the big data set
		
		for(int i=0;i<13;i++) {
			newdata.push_back(sample_datum(1));
			newdata.push_back(sample_datum(2));
			for(int j=0;j<3;j++){
				newdata.push_back(sample_datum(3));
				newdata.push_back(sample_datum(4));
				newdata.push_back(sample_datum(5));
			}
			newdata.push_back(sample_datum(6));
			newdata.push_back(sample_datum(6));
		}
		
		all.compute_posterior(newdata).print("orear\t");
	}	
