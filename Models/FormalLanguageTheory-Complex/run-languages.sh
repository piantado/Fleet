...
myhost=$(hostname -s)
threads=2 # for running on simon and garfunkel, multithreads don't seem to go well
jobs=30 # on the other hand, we can have lots of jobs
...
#   cat Parameters.csv | sed "s/$/,${factors[2]}/" ;
#   cat Parameters.csv | sed "s/$/,${factors[3]}/" ; } | 
