...
{ cat Parameters.csv | sed "s/$/,${factors[0]}/" ; 
  cat Parameters.csv | sed "s/$/,${factors[1]}/" ; } | 
        parallel --jobs=16 --joblog log.$myhost --csv --colsep ',' \
        /usr/bin/time --output=out/{1}-{5}.time --verbose \
        ./$mymain "${GARGS[@]}" --input=data/{1} --alphabet=\"{2}\" --time={3} --nfactors={5} --thin=0 --mcmc=0 --top=100 --restart=100000 ">" out/{1}-{5}.out "2>" out/{1}-{5}.err
