...
namespace Model {
	...
	double ANSdiff(const double n1, const double n2) {  // SD of the 
		return (n1-n2) / (W*sqrt(n1*n1+n2*n2));
	}
	bool ApproxEq(const set s, const set s2) {
		return abs(ANSdiff(s.length(), s2.length())) < 1.0; // we'll call differences w/in 1 SD equal 		
	}
	bool ApproxEq(const set s, const wmset s2) {
		return abs(ANSdiff(s.length(), s2)) < 1.0; // we'll call differences w/in 1 SD equal 		
	}
	bool ApproxEq(const set s, const magnitude m) {
		return  abs(ANSdiff(s.length(), m)) < 1.0; // we'll call differences w/in 1 SD equal 		
	}
