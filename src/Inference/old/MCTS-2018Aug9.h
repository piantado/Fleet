...
public:
	...
	std::atomic<size_t> upmax; // how many times did I increase my parent's maximum?
    ...
    int sample_child() {
		...
		// this version requires us to fill in the whole tree, which is crazy
//		if(numunsearched > 0) { 
//			size_t mx = 0;
//			for(auto const c: children){
        ...
        for(auto const c: children) {
            if(c->open){ 
				...
				if(this->maxp > -infinity) {
					...
					// This normalized score is not good -- if you're at a branch and you run your samples, you don't get more if you discover an improvement
					// what you want is a score part that is NOT bounded in 0/1 so that the more your improve, the more samples you get
					//score += exp( c->maxp - this->maxp);
					score += c->maxp - this->maxp;
					
					
					
					// A "win" here should be how many times did you find a structure that was higher than the other nodes?
					// or a "win" is increasing the max?
					
					
					
					//score += c->totalp - log(c->nsamples+1); // total amount of probability mass per sample
					// sumlogp doesn't make sense because its often -inf
					
					// These are similarly bad for being bounded:
					//score += exp(c->totalp - this->totalp); // proportional to conditional amount of mass found -- but this doesn't count the number of samples?
					//score += exp(c->totalp - log((double)c->nsamples) - this->totalp + log((double)this->nsamples) ); // average probability mass per sample
					//score += exp(c->sumlogp / c->nsamples - this->maxp); //exp( c->maxp - this->maxp);
				}
				else {
					score = -infinity;
				}
...
//		// NOTE: We can't do this for upmax, since that has to do with the online sampling...
//		
...
//			numunsearched = 0;
//            for(auto c: children) {
//				if(c->open) { // let's work only over open children, since otherwise our search is biased by stuff we can't find (closed nodes can't guide future search)
//					
//					totalp = logplusexp(c->totalp, totalp); 
...
//    }
    ...
    void add_sample(const double v, const size_t num){ // found something with v
		...
		if(std::isnan(v) || std::isinf(v)) return; // just ignore these
		...
		if(parent != nullptr && v > parent->maxp ) {
			upmax++; // well then I'm going to increase my parent's max
		}
    ...
    void search_one() {
		...
		if(nsamples == 0) { // I am a leaf of the search who has not been expanded yet
			...
			// In this version, when we add child nodes, we expand all of them!
...
//					if(c->open){						
//						c->nsamples++;
//						double score = compute_playouts(c->value); // compute the playouts with c as the node
//						c->add_sample(score,1); // update my internal counts
//						c->open = value->is_partial(); // only open if the value is partial
//						c->recompute_stats(); 
//					}
//				}
//				
//				pthread_mutex_unlock(&child_modification_lock);
//				
//				return; // this counts as a serch move
			}
