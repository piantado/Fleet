...
/**
 * @brief What order am I in a partial subtree enumeration from x?
 * @param g
 * @param n
 * @param is
 */
template<typename Grammar_t>
[[nodiscard]] enumerationidx_t compute_partial_subtree_order(Grammar_t* g, const Node& x, const Node& n) {
	...
	if(n.is_null()) {
		...
		for(int i=N-1;i>=0;i--) {
			...
			// the most we can pop from a child is the number of partial trees they have
			auto cx = count_partial_subtrees(x.child(i));
			auto k = compute_partial_subtree_order(g, x.child(i), n.child(i));
...
//			
//			// Problem here is that there are two encodings of null -- 
//			
//			
//			// the most we can pop from a child is the number of partial trees they have
//			auto cz = count_partial_subtrees(n.child(i));
